...
public class Array2D<T> {
    ...
    public static Array2D<T> generated(int width, int height, System.Func<Vector2Int, T> generator)
    {
        ...
        if (width * height > 0)
        {
            for (int i = 0; i < width * height; ++i)
                a2._elements.Add(generator(Vector2Int.zero));
        }

        // Don't call generator() on the first cell twice.
        for (var x = 1; x < width; x++) {
            a2._set(x, 0, generator(new Vector2Int(x, 0)));
        }

        for (var y = 1; y < height; y++) {
            for (var x = 0; x < width; x++) {
                a2._set(x, y, generator(new Vector2Int(x, y)));
            }
        }
